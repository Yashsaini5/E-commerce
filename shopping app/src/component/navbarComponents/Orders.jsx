...
const Orders = () => {
  ...
  const loadOrders = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:5000/api/order/userOrders", {  withCredentials: true,})
  ...
  return (
    <div>
      ...
      <div className='p-10 bg-gray-200 flex flex-col min-h-screen'>
        ...
        <p className='text-2xl w-full h-12 font-semibold border-b-4 border-gray-500'>My Orders</p>
        {orders.length === 0 ? (
      ...
      >
        ...
        <div className="mt-4 sm:mt-0 sm:w-64 space-y-2 text-sm">
          <div>
            ...
            <div className='flex gap-2'><p>{order.shippingAddress.firstName}</p>
            <p>{order.shippingAddress.lastName}</p></div>
