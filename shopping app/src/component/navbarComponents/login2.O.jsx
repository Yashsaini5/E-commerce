...
const SocialIcons = () => (
  <div className="flex justify-center gap-4 mt-8">
    {["google", "facebook", "github", "linkedin"].map((icon) => (
      <span
        key={icon}
        className="h-10 w-10 bg-slate-200 rounded-md flex items-center justify-center text-base hover:text-xl hover:border-sky-500 hover:ring-2"
      >
...
const LoginPage = () => {
  ...
  useGSAP(() => {
    Object.keys(gsapRefs).forEach((key) => {
      gsap.to(gsapRefs[key].current, {
        x: animState[`${key.replace("gsap", "").toLowerCase()}X`],
        duration: 1.5,
        zIndex: animState[`${key.replace("gsap", "").toLowerCase()}Z`],
        ease: "sine.inOut",
      });
    });
  }, [animState]);
  ...
  return (
    <div className="h-[100vh] w-full flex justify-center items-center bg-slate-900">
      <div className="box h-[70vh] w-[70vw] flex bg-transparent relative mt-16">
        <div
          ref={gsapRefs.gsapLogin}
          className="login h-full bg-white w-1/2 flex flex-col justify-center z-10"
        >
        ...
        <div
          ref={gsapRefs.gsapCreate}
          className="signup h-full bg-white w-1/2 flex flex-col justify-center absolute left-0"
        >
          ...
          <div className="flex justify-center mt-4 mb-2">
            <GlowingButton
              onClick={(e) => handleSubmit(e, "createUser")}
              text="SIGN UP"
            />
          </div>
        </div>
